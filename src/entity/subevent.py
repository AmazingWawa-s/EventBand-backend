...
#更新event数据库中关于此活动的信息-------------------------------------------------------   
    ...
    @staticmethod
    def selectUidById(rid):
        dbop=CostremarkDB()
        dbop.selectRemarksById("cr_user_id",rid)
        return dbop.get()
    
    @staticmethod
    def selectEidById(rid):
        dbop=CostremarkDB()
        dbop.selectRemarksById("cr_event_id",rid)
        return dbop.get()
    
    def getFromDB(self):
        dbop=CostremarkDB()
        dbop.selectRemarksById("*",self.id)
        result=dbop.get()
        if len(result)==1:
            self.set(result[0])
        elif len(result)==0:
            raise ValueError("CostremarkId Not Exist")
        else :raise ValueError("Costremark getFromDB Error")